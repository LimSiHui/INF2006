				...
				<tbody>
					...
					$i = 1;
						...
						if ($is_admin) {
							...
							$stmt = $conn->prepare("SELECT r.*, c.category, c.balance FROM `running_balance` r INNER JOIN `categories` c ON r.category_id = c.id WHERE c.status = 1 AND r.balance_type = 2 ORDER BY unix_timestamp(r.date_created) DESC");
						} else {
							// Prepare statement for non-admin
							$current_user_id = $_settings->userdata('id'); // Or however you get the current user's ID
							$stmt = $conn->prepare("SELECT r.*, c.category, c.balance FROM `running_balance` r INNER JOIN `categories` c ON r.category_id = c.id WHERE c.status = 1 AND r.balance_type = 2 AND r.user_id = ? ORDER BY unix_timestamp(r.date_created) DESC");
